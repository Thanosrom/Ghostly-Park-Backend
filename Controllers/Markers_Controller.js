...
const Markers_Controller = {
    ...
    async send_UnParked_Location(req, res) {
        ...
        try{
          const results = await Markers_Model.send_UnParked_Location_Model(req.user.id,lat,lng);
          if(results){
              res.sendStatus(200);
          }
          else{
    ...
    async send_Parked_Location(req, res) {
        ...
        try{
          const results = await Markers_Model.send_Parked_Location_Model(req.user.id, parked_long, parked_lat);
        	if(results){
        	res.sendStatus(200);
        }
        else{
    ...
    async get_Parked_Location(req, res) {
      ...
      try{
        const results = await Markers_Model.get_Parked_Location_Model(req.user.id);
        if(results){
		      res.status(200).json(await Markers_Model.get_Parked_Location_Model(req.user.id));
      }
      else{
            res.sendStatus(400);
          }
