...
async function send_UnParked_Location_Model(user,lat,lng){
    ...
    try {
        const [results] = await dbConnection.execute('INSERT INTO unparked_slots (user, unParked_latitude, unParked_longitude, unParked_time) VALUES (?, ?, ?, ?)', [
            user,
            lat,
            lng,
            timestamp,
...
async function send_Parked_Location_Model(user, parked_long, parked_lat){
    ...
    try {
	    await dbConnection.execute('DELETE FROM parked_slots WHERE user = ?',[user]);
        const [results] = await dbConnection.execute('INSERT INTO parked_slots (user, parked_longitude, parked_latitude, parked_time) VALUES (?, ?, ?, ?)', [
            user,
            parked_long,
            parked_lat,
            parked_time,
...
async function get_Parked_Location_Model(user) {
    ...
    try {
        const [results] = await dbConnection.execute('SELECT * FROM parked_slots WHERE user = ?',[user]);
        return results;
    }catch(error){   
        return false;
    }finally {
...
async function delete_Markers_Model(){
    ...
    try {
        const [rows] = await dbConnection.execute('SELECT unParked_time FROM unparked_slots');
          
        const hours_Ago = moment().subtract(12, 'hours'); // 12 hour ago
        
        //Do it as a timestamp
        const markersToDelete = rows.filter(marker => moment.unix(marker.unParked_time / 1000).isBefore(hours_Ago));
    
        // Delete the old markers from the database
        for (const marker of markersToDelete) {
          await dbConnection.query('DELETE FROM unparked_slots WHERE unParked_time = ?', [
